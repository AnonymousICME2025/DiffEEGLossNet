import torch
from torch.nn import Module, Conv2d, ModuleList, Sequential, Linear, SiLU, Embedding
from torch.nn.functional import one_hot
from erd_ers_feature import calculate_erd_ers, calculate_erd_ers_loss  # 导入ERD/ERS特征计算
...
class DiffEEGLossNet(Module):
    ...
    def forward(self,x:torch.Tensor,lambd:torch.Tensor,
        ...
        erd_ers_features = calculate_erd_ers(x)  # (B, E, L)
        
        # Integrate ERD/ERS into the model by adding loss
        erd_ers_loss = calculate_erd_ers_loss(erd_ers_real=erd_ers_features, erd_ers_generated=x)  # Loss based on ERD/ERS
        
        # Continue the forward pass using ERD/ERS enhanced features
        ...
        return output, erd_ers_loss  # Returning the output and ERD/ERS loss
